...
func wechatExchange(ctx context.Context, p systemcodec.OauthProfile, code, _ string, _ FlowSecrets) (ExternalIdentity, error) {
	...
	if err := wechatGet(ctx, tokenURL, &token); err != nil {
		return ExternalIdentity{}, fmt.Errorf("wechat token exchange: %w", err)
	}
	if token.ErrCode != 0 {
		return ExternalIdentity{}, fmt.Errorf("wechat token exchange: %s (%d)", token.ErrMsg, token.ErrCode)
	}
	...
	if err := wechatGet(ctx, infoURL, &info); err != nil {
		return ExternalIdentity{}, fmt.Errorf("wechat userinfo: %w", err)
	}
	if info.ErrCode != 0 {
		return ExternalIdentity{}, fmt.Errorf("wechat userinfo: %s (%d)", info.ErrMsg, info.ErrCode)
	}
	...
	if subject == "" {
		return ExternalIdentity{}, fmt.Errorf("wechat userinfo: no unionid or openid")
	}
