...
func (s *systemStorage) CreateStorageProfile(ctx context.Context, req *connect.Request[systemv1.CreateStorageProfileRequest]) (*connect.Response[systemv1.CreateStorageProfileResponse], error) {
	...
	canonical, err := s.storageRegistry.CreateConfig(input.GetProvider(), configValues(input.GetConfig()), input.GetConfig().GetSecrets())
	if err != nil {
...
func (s *systemStorage) UpdateStorageProfile(ctx context.Context, req *connect.Request[systemv1.UpdateStorageProfileRequest]) (*connect.Response[systemv1.UpdateStorageProfileResponse], error) {
	...
	for i, stored := range cfg.Profiles {
		...
		if stored.Provider != input.GetProvider() {
			return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("storage provider cannot be changed"))
		}
		canonical, err := s.storageRegistry.UpdateConfig(stored.Provider, configValues(input.GetConfig()), input.GetConfig().GetSecrets(), stored.Config)
		if err != nil {
			return nil, connect.NewError(connect.CodeInvalidArgument, err)
		}
		...
		return connect.NewResponse(&systemv1.UpdateStorageProfileResponse{Profile: s.storageProfileToProto(profile)}), nil
	}
...
func (s *systemStorage) TestStorageConnection(ctx context.Context, req *connect.Request[systemv1.TestStorageConnectionRequest]) (*connect.Response[systemv1.TestStorageConnectionResponse], error) {
	...
	if err := s.storageTester.TestConnection(ctx, profile); err != nil {
		return connect.NewResponse(&systemv1.TestStorageConnectionResponse{Ok: false, Message: testFailureMessage(err, storage.ErrNotConfigured, "storage is not configured: fill in the connection settings")}), nil
	}
...
func (s *systemStorage) resolveStorageTestProfile(ctx context.Context, input *systemv1.ProfileInput, id string) (kitsettings.GenericProfile, error) {
	...
	if input != nil {
		if stored, ok := cfg.Profile(input.GetId()); ok {
			if stored.Provider != input.GetProvider() {
				return kitsettings.GenericProfile{}, connect.NewError(connect.CodeInvalidArgument, errors.New("storage provider cannot be changed"))
			}
			canonical, err := s.storageRegistry.UpdateConfig(stored.Provider, configValues(input.GetConfig()), input.GetConfig().GetSecrets(), stored.Config)
			if err != nil {
...
func (s *systemStorage) toProtoStorage(cfg settings.Storage) *systemv1.StorageSettings {
	...
	for i, purpose := range storage.Purposes {
		bindings[i] = &systemv1.StorageBinding{Purpose: purpose.Key, ProfileId: firstID(cfg.Bindings[purpose.Key])}
	}
