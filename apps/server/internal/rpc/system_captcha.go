...
func (s *SystemService) CreateCaptchaProfile(ctx context.Context, req *connect.Request[systemv1.CreateCaptchaProfileRequest]) (*connect.Response[systemv1.CreateCaptchaProfileResponse], error) {
	...
	canonical, err := s.captchaRegistry.CreateConfig(in.GetProvider(), configValues(in.GetConfig()), in.GetConfig().GetSecrets())
	if err != nil {
...
func (s *SystemService) UpdateCaptchaProfile(ctx context.Context, req *connect.Request[systemv1.UpdateCaptchaProfileRequest]) (*connect.Response[systemv1.UpdateCaptchaProfileResponse], error) {
	...
	for i, old := range cfg.Profiles {
		...
		if old.Provider != in.GetProvider() {
			return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("captcha provider cannot be changed"))
		}
		canonical, err := s.captchaRegistry.UpdateConfig(old.Provider, configValues(in.GetConfig()), in.GetConfig().GetSecrets(), old.Config)
		if err != nil {
			return nil, connect.NewError(connect.CodeInvalidArgument, err)
		}
		...
		return connect.NewResponse(&systemv1.UpdateCaptchaProfileResponse{Profile: s.captchaProfileToProto(p)}), nil
	}
...
func (s *SystemService) toProtoCaptcha(cfg settings.Captcha) *systemv1.CaptchaSettings {
	...
	for i, p := range captcha.Purposes {
		bindings[i] = &systemv1.CaptchaBinding{Purpose: p.Key, ProfileId: firstID(cfg.Bindings[p.Key])}
	}
