	...
	store := fakeStore{
		cfg: Config{
			Profiles: []kitsettings.GenericProfile{
				{Id: "p", Provider: "altcha", Config: map[string]any{"difficulty": 1000}},
			},
			Bindings: map[string][]string{"auth": {"p"}},
		},
		key: []byte("test-altcha-key"),
	}
...
func solve(t *testing.T, c *Client) string {
	...
	c.ServeAltchaChallenge(
		rec,
		httptest.NewRequest("GET", "/captcha/altcha/challenge?purpose=auth", nil),
	)
	if rec.Code != 200 {
...
func TestUnboundCaptchaPassesButInvalidBoundProfileFailsClosed(t *testing.T) {
	...
	bad := fakeStore{
		cfg: Config{
			Profiles: []kitsettings.GenericProfile{
				{Id: "p", Provider: "turnstile", Config: map[string]any{"siteKey": "site"}},
			},
			Bindings: map[string][]string{"auth": {"p"}},
		},
	}
