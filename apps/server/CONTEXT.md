...
**purpose（用途）**：
base/application 对某类 integration 能力提出的固定需求槽位（如头像存储、认证邮件、验证码、结算）；具体 purpose key、展示文案及单值/多值绑定基数归 base catalog，driver 只接受调用，不定义业务为何调用。Catalog 同时驱动写侧绑定校验与管理端 descriptor。
_Avoid_: 把具体 purpose 放进可复用 driver 包；让 provider 决定应用有哪些用途；在前端维护 purpose→文案映射。
...
**config schema（配置描述）**：
driver 的 profile config 契约：在 form schema 之上增加 `secret`/`immutable`，由 base 统一执行 Mask/Merge/Validate/Usable，前端只消费生成的表单。照 Terraform provider schema 塑形。见 ADR-0006。
_Avoid_: 让 product input 继承密钥保留语义；把 config 形状写进中央 proto；让 driver 自己实现掩码。

**config（配置值）**：
一个 profile 的连接参数**取值**，wire 上以 opaque `Struct` 走；形状由 driver 的 config schema 定义，语义只有 driver 懂。base 不懂 config 语义，只按 schema 的结构标志处理它。
