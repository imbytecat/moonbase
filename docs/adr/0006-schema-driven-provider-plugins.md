...
- **前端**：不引 Formily / rjsf / SurveyJS；用已装的 `@tanstack/react-form` + 一个薄的 `descriptor→antd` 渲染器（一个可 grep 的 `switch`）。前端校验只当 UX，**真正的闸是 base 的 `Validate`**（服务端不信任客户端）。
...
- **前端上重型 schema 引擎（Formily / rjsf / SurveyJS）**——否决。antd v6 不匹配（rjsf 主题停在 v5、Formily 停更且绑定滞后）+ 过重 + 反 grep。closed descriptor 用薄渲染器即可；schema 若日后长出深嵌套 / 条件联动再重评。
