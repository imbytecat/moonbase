...
**例外——固定目录枚举**：当某字段的选项值来自一个代码生成的 proto 枚举、且其显示文案已作为前端「固定枚举→中文」目录存在（与同页 `provider`/`status` 目录并列，如 payment 的 `methods` ↔ `payments.ts` 的 `methodLabel`/`methodDesc`），driver 只发 `value`，显示文案仍由前端目录提供，设置表单对该字段单独渲染。理由：这类文案是固定枚举的展示、非 per-provider 自由配置；把它搬进 driver 会与其兄弟目录（`PROVIDER_LABELS`/`STATUS_LABELS` 仍手写在前端）割裂成两套模式，且要改自定义生成器 `protoc-gen-paymentcatalog`。`encryption`/`authMethod` 这类自由 driver 枚举不属此列，照常 driver 拥有。
