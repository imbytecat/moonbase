# 采用 rjsf 渲染前端配置表单：Go `[]Field` 为单一真源，base 生成 JSON Schema

> **状态**：accepted。**推翻 ADR-0006**「前端薄渲染器、不引 rjsf」；**推翻 ADR-0009 的 proto `OptionDescriptor`**（选项含义仍归 driver，但改由 Go `schema.Field` 承载、不再走 proto 消息）；**保留 ADR-0006 后端内核**（driver 拥有 config schema、`Mask/Merge/Validate/Usable` 通用引擎、掩码单点不变量、编译期 driver registry）。
...
- **secret 写-only 掩码保留**（后端对 config **值**的行为，与「wire 传 JSON Schema」正交）；前端以 rjsf custom widget 渲染（password + 「留空保持不变」+ `_set`）。
