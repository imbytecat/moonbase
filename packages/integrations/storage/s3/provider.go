...
func New() storageint.Registration {
	return storageint.Register(
		...
		integration.Presentation{
			Name:        "S3 兼容存储",
			Description: "连接兼容 S3 协议的对象存储服务",
			Color:       "#1677ff",
			IconRef:     "antd:CloudServerOutlined",
		},
		config.MustContract[providerConfig](config.Policy{Secrets: []string{"/secretAccessKey"}}),
...
func presignPut(
	...
	u, err := mc.Presign(
		...
		key,
		expires,
		url.Values{"Content-Type": {contentType}},
	)
	if err != nil {
