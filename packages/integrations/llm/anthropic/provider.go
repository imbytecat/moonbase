...
func New() llmint.Registration {
	return llmint.Register("anthropic", integration.Presentation{Name: "Anthropic 模型", Description: "连接 Anthropic 消息协议的对话模型", Color: "#d97757", IconRef: "antd:AnthropicFilled"}, config.MustContract[providerConfig](config.Policy{Secrets: []string{"/apiKey"}}), complete)
}
func complete(ctx context.Context, c providerConfig, p llmint.Prompt) (string, error) {
	...
	params := sdk.MessageNewParams{Model: sdk.Model(c.Model), MaxTokens: 1024, Messages: []sdk.MessageParam{sdk.NewUserMessage(sdk.NewTextBlock(p.User))}}
	if p.System != "" {
