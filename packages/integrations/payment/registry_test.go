...
func testRegistration(create func(context.Context, registryConfig, CreateRequest) (Action, error)) Registration {
...
func TestRegistryRejectsInvalidConfigBeforeExecution(t *testing.T) {
	called := false
	registry := MustRegistry(testRegistration(func(_ context.Context, cfg registryConfig, _ CreateRequest) (Action, error) {
		called = cfg.APIKey != ""
		return Action{}, nil
	}))
...
func TestRegistryProjectsAndKeepsSecrets(t *testing.T) {
	registry := MustRegistry(testRegistration(func(context.Context, registryConfig, CreateRequest) (Action, error) { return Action{}, nil }))
	created, err := registry.CreateConfig("test", map[string]any{"endpoint": "https://pay.example.test"}, map[string]string{"/apiKey": "secret"})
	if err != nil {
		t.Fatal(err)
	}
	updated, err := registry.UpdateConfig("test", map[string]any{"endpoint": "https://new.example.test"}, nil, created)
	if err != nil {
		t.Fatal(err)
	}
	view, valid := registry.ViewConfig("test", updated)
	if !valid || view.Values["apiKey"] != nil || !slices.Equal(view.SetSecretPaths, []string{"/apiKey"}) {
...
func TestRegistryDispatchesTypedOperations(t *testing.T) {
	...
	registry := MustRegistry(RegisterOperations("test", integration.Presentation{Name: "测试支付"},
		config.MustContract[registryConfig](config.Policy{Secrets: []string{"/apiKey"}}), ProviderDescriptor{},
