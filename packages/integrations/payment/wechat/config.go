...
var descriptor = pay.ProviderDescriptor{
	...
	Products: []pay.ProductDescriptor{
		{ID: "native", Method: "wechat", Presentation: integration.Presentation{Name: "Native 支付", Description: "由付款人扫码完成支付"}},
		{ID: "h5", Method: "wechat", Presentation: integration.Presentation{Name: "H5 支付", Description: "跳转到移动端支付页面"}},
		{ID: "jsapi", Method: "wechat", Presentation: integration.Presentation{Name: "JSAPI 支付", Description: "在微信内唤起支付"}, Input: form.Schema{Fields: []form.Field{{Key: "payer_id", Label: "用户标识", Type: form.String, Required: true, MaxLen: 128}}}},
		{ID: "app", Method: "wechat", Presentation: integration.Presentation{Name: "App 支付", Description: "在移动应用内唤起支付"}},
	},
}
...
func (providerConfig) JSONSchemaExtend(schema *invopop.Schema) {
	if authMethod, ok := schema.Properties.Get("authMethod"); ok {
		authMethod.Enum = []any{pay.AuthPublicKey, pay.AuthPlatformCert}
	}
...
func New() pay.Registration {
	return pay.RegisterOperations(
		"wechat",
		integration.Presentation{Name: "微信支付", Description: "微信支付直连商户", Color: "#07c160", IconRef: "antd:WechatOutlined"},
		config.MustContract[providerConfig](config.Policy{Secrets: []string{"/mchPrivateKey", "/apiV3Key", "/publicKey"}}),
		descriptor,
		pay.Operations[providerConfig]{
			...
			Query: wechatQuery, Refund: wechatRefund, QueryRefund: wechatQueryRefund,
